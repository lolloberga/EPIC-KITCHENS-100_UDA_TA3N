...
class VideoModel(nn.Module):
    ...
    def _prepare_DA(self, num_class, base_model, modality):  # convert the model to DA framework
        ...
        if self.frame_aggregation == 'rnn':  # 2. rnn
            ...
            if self.rnn_cell == 'LSTM':
                self.rnn = nn.LSTM(feat_frame_dim, self.hidden_dim // self.n_directions, self.n_layers, batch_first=True,
    ...
    def forward(self, input_source, input_target, beta, mu, is_train, reverse):
        ...
        pred_fc_target = (
            self.fc_classifier_target_verb(feat_fc_target) if self.share_params == 'N' else self.fc_classifier_source_verb(
                feat_fc_target),
            self.fc_classifier_target_noun(feat_fc_target) if self.share_params == 'N' else self.fc_classifier_source_noun(
                feat_fc_target))
        ...
        if self.frame_aggregation == 'avgpool' or self.frame_aggregation == 'rnn':
            ...
            if self.use_bn != 'none':
                feat_fc_video_source_3_1, feat_fc_video_target_3_1 = self.domainAlign(feat_fc_video_source_3_1,
                                                                                      feat_fc_video_target_3_1, is_train,
