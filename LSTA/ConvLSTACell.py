...
class ConvLSTACell(nn.Module):
    ...
    def forward(self, x, state_att, state_inp, x_flow_i=0, x_flow_f=0, x_flow_c=0, x_flow_o=0):
        # state_att = [a, s]
        # state_inp = [atanh(c), o]
        ...
        i_s = F.sigmoid(self.conv_i_s(s_t_1))
        f_s = F.sigmoid(self.conv_f_s(s_t_1))
        o_s = F.sigmoid(self.conv_o_s(s_t_1))
        a_tilde = F.tanh(self.conv_a_s(s_t_1))
        a = (f_s * a_t_1) + (i_s * a_tilde)
        s = o_s * F.tanh(a)
        u = s  # hidden state + cam
