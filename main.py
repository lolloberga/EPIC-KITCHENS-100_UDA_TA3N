...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # train for one epoch
        loss_c, attn_epoch_source, attn_epoch_target = train(num_class, source_loader, target_loader, model, criterion,
                                                             criterion_domain, optimizer, epoch, train_file,
                                                             train_short_file, alpha, beta, gamma, mu)
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short,
    ...
    for i, ((source_data, source_label, source_id), (target_data, target_label, target_id)) in data_loader:
        ...
        if args.pretrain_source:
            ...
            _, out_source, out_source_2, _, _, _, _, _, _, _ = model(source_data, target_data, beta_new, mu,
                ...
                for l in range(0,
                    ...
                    if args.place_dis[l] == 'Y':
                        ...
                        if args.dis_DA == 'CORAL':
                            losses_coral = [CORAL(feat_source_sel[t], feat_target_sel[t]) for t in
                                            range(feat_source_sel.size(0))]
    ...
    for i, (val_data, val_label, _) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            if args.baseline_type == 'tsn':
                ...
                pred_noun = pred_noun.view(val_label.size(0), -1, num_class).mean(
                    dim=1)  # average all the segments (needed when num_segments != val_segments)
